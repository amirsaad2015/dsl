    ...
    TerminalBackendContribution.prototype.onStart = function (server) {
        ...
        node_2.openSocket({
            ...
            matches: function (request) {
                ...
                return uri.path.toString().startsWith(terminal_protocol_1.terminalsPath + "/");
            }
        }, function (ws, request) {
            var uri = new uri_1.default(request.url);
            ...
            var output = termProcess.createOutputStream();
            output.on('data', function (data) {
                try {
                    ws.send(data);
                }
                catch (ex) {
                    _this.logger.warn(ex.message, ex);
                }
            });
            ws.on('error', function (err) {
                _this.logger.warn(err.message, err);
            });
