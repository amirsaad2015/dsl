...
var CallHierarchyTreeWidget = /** @class */ (function (_super) {
    ...
    function CallHierarchyTreeWidget(props, model, contextMenuRenderer, labelProvider, editorManager) {
        ...
        _this.title.iconClass = 'fa fa-arrow-circle-down';
        _this.title.closable = true;
        _this.addClass(exports.HIERARCHY_TREE_CLASS);
        _this.model.onSelectionChanged(function (node) {
            if (node) {
                _this.openEditor(node, true);
            }
        });
        _this.model.onOpenNode(function (node) {
    ...
    CallHierarchyTreeWidget.prototype.onUpdateRequest = function (msg) {
    ...
    CallHierarchyTreeWidget.prototype.renderTree = function (model) {
        return _super.prototype.renderTree.call(this, model)
            || virtualdom_1.h.div({ className: 'noCallers' }, 'No callers have been detected.');
    };
    ...
    CallHierarchyTreeWidget.prototype.decorateDefinitionCaption = function (definition) {
        ...
        var icon = virtualdom_1.h.div({ className: "symbol-icon " + this.toIconClass(definition.symbolKind) });
        var symbol = definition.symbolName;
        var symbolElement = virtualdom_1.h.div({ className: 'symbol' }, symbol);
        ...
        var containerElement = virtualdom_1.h.div({ className: 'container' }, container);
        return virtualdom_1.h.div({ className: 'definitionNode' }, icon, symbolElement, containerElement);
    };
    CallHierarchyTreeWidget.prototype.decorateCallerCaption = function (caller) {
        ...
        var referenceCountElement = virtualdom_1.h.div({ className: 'referenceCount' }, (referenceCount > 1) ? "[" + referenceCount + "]" : '');
        var location = this.labelProvider.getName(new uri_1.default(definition.location.uri));
        var container = (containerName) ? containerName + ' — ' + location : location;
        var containerElement = virtualdom_1.h.div({ className: 'container' }, container);
        return virtualdom_1.h.div({ className: 'definitionNode' }, icon, symbolElement, referenceCountElement, containerElement);
    };
